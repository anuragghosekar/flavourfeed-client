...
export default function UserRecipes() {
  ...
  const deleteRecipe = async (recipeId) => {
    try {
      ...
      setPostedRecipes(postedRecipes.filter(recipe => recipe.recipeId !== recipeId));
    } catch (error) {
    ...
    <>
      ...
      <div className="container user-recipes-container">
        ...
        <div className="user-recipes">
          {postedRecipes && postedRecipes.map((recipe) => (
        ...
        <Link to="/addnewrecipe" className="btn btn-success add-new-recipe-btn">Add New Recipe</Link>
      </div>
    </>
  );
}
